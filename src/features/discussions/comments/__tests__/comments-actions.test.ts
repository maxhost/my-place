...
import { EditWindowExpired, InvalidQuoteTarget } from '@/features/discussions/domain/errors'
...
// `findIsPlaceAdmin` consulta `groupMembership.findFirst` filtrado por preset.
// `hasPermission` consulta `groupMembership.findMany` filtrado por permiso.
// Defaults reflejan "no es admin". `mockActiveMember(ADMIN)` activa ambas.
...
function mockActiveMember(opts: { asAdmin?: boolean } = {}): void {
  ...
  // (b) `hasPermission.findMany` → grupo con TODOS los permisos.
  if (opts.asAdmin) {
...
beforeEach(() => {
  vi.resetAllMocks()
  // C.3: re-instalar defaults que `resetAllMocks` borra.
  groupMembershipFindFirst.mockResolvedValue(null)
  groupMembershipFindMany.mockResolvedValue([])
