...
export async function updateEventAction(input: unknown): Promise<{ ok: true }> {
  ...
  // G.3: gate atómico permission-groups. Author siempre puede; mods con
  // `events:moderate` pueden editar ajenos.
  ...
  if (event.authorUserId !== actor.actorId && !canModerate) {
    throw new AuthorizationError('No tenés permiso para modificar este evento.', {
      eventId: event.id,
