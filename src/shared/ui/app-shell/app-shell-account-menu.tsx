...
// Account menu del shell agnóstico (ADR-0023). Dropdown anclado al avatar
// de la topbar. V1 sólo expone "Cerrar sesión" (Server Action inyectada
// como prop — patrón seam-split del repo: la UI no toca el SDK, recibe la
// acción y la invoca con un fake en tests). Promovido a `shared/ui/app-shell/`
// desde el histórico `nav-hub/ui/account-menu.tsx` del Hub V1; misma
// semántica, mismo Avatar fallback.
//
// `onLogout` retorna `{redirectTo}`; el componente navega con
// `window.location.assign(redirectTo)` por default (cross-subdomain seguro
// — el server no hace `redirect()` para evitar fricciones de Next 16 con
// orígenes distintos). En tests se inyecta `navigate={vi.fn()}` para
// verificar la URL — jsdom no permite spyOn estable sobre
// `window.location.assign`. Durante el pending el item se renderea con
// label alternativo y `aria-disabled="true"` para evitar doble click.
...
type Props = {
   ...
   * sólo en tests. Producción usa el default sin pasarlo.
   */
...
export function AppShellAccountMenu({
  ...
  // mousedown (no click) garantiza que cerramos antes de que el target
  // afuera dispare su propio onClick (e.g., otro botón ajeno al menú).
  useEffect(() => {
    if (!isOpen) return;
...
function Avatar({ displayName }: { displayName: string | null }) {
  ...
  if (!initials) {
    // Fallback: círculo neutro con ícono genérico — no exponemos un emoji
    // ni texto si no tenemos nombre (defensive: el caso "displayName null"
    // no debería pasar en V1, pero el componente lo absorbe sin romper).
    return (
      <svg
        width={20}
        height={20}
