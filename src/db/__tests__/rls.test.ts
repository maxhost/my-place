...
// `place_domain` movido a `rls-place-domain.test.ts` con matriz exhaustiva +
// structural drift defense. Los tests cross-table de aislamiento siguen
// cubriendo place/place_ownership/membership/invitation.
const PLACE_TABLES = ["place", "place_ownership", "membership", "invitation"] as const;
...
describe("S2 RLS — aislamiento por place (ADR-0010/0012)", () => {
  it("el owner ve exactamente las filas de su place, sin recursión", async () => {
    await inRlsTx(async (tx) => {
      await seedPlaceA(tx);
      await tx.as("authA");
      // place/membership/invitation sub-consultan place_ownership; si el
      // fraseo no fuese recursion-safe esto lanzaría `infinite recursion`.
...
describe("S2 RLS — invitation owner-only (ADR-0012 §2)", () => {
  it("el owner SÍ inserta invitation de su place", async () => {
    await inRlsTx(async (tx) => {
      const { pid } = await seedPlaceA(tx);
      await tx.as("authA");
      expect(
  ...
  it("un no-owner (B) NO inserta invitation en place ajeno", async () => {
    await inRlsTx(async (tx) => {
      const { pid } = await seedPlaceA(tx);
      await tx.as("authB");
      expect(
