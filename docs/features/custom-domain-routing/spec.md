...
**Auth gap V1**. La cookie de sesión Neon Auth está scopeada a `Domain=.place.community` (decisión del SDK, ADR-0001 §"Sesión y SSO"). Un custom domain como `nocodecompany.co` es host-only — el browser NO envía la cookie del apex. En consecuencia, owners autenticados en `place.community` NO tienen sesión local en su custom domain. Feature B documenta y cubre este gap V1 con un **gate page educativo server-side** (ver §Gate page abajo); el cierre estructural (**Signed Ticket SSO** + JWT host-only del custom domain en cookie `__Host-place_sso_session` + silent redirect chain init→issue→redeem) está entregado en Feature C V1 (slice [`docs/features/custom-domain-sso/`](../custom-domain-sso/spec.md), ADR-0032). El gate page V1 de Feature B queda como CTA fallback dentro de `<SsoFallbackPanel>` cuando el silent SSO falla.

**Relación con Feature C**. Feature B dejó el proxy `async` (wrapper sobre el `resolveHost` sync) y el shape jsonb del lookup incluye `place_id`. Feature C V1 (ADR-0032) consume estos pilares sin modificar el schema de `place_domain`: el slice `custom-domain-routing` recibió `<SsoFallbackPanel>` como componente nuevo (S6), y `<AuthGateForCustomDomain>` quedó como CTA fallback dentro del panel. Los endpoints SSO (`/api/auth/sso-init`, `/api/auth/sso-redeem`) viven en `src/app/api/auth/sso-*/` — fuera del slice routing. El gap auth se cerró sin refactor estructural del slice (sólo se agregó un componente nuevo).
...
- **SSO desde custom domain**: cubierto por **Feature C V1** (ADR-0032, **Signed Ticket pattern**). Slice canónico: [`docs/features/custom-domain-sso/`](../custom-domain-sso/spec.md). NO usa Better Auth OIDC Provider plugin (no accesible desde Neon Auth managed). NO hay callback handler OIDC. Endpoints reales: `/api/auth/sso-init` + `/api/auth/sso-issue` (apex) + `/api/auth/sso-redeem`. Cookie host-only `__Host-place_sso_session` con JWT ES256 firmado por apex. "Silent" = server-side redirect chain sin interacción cuando el owner tiene sesión Neon Auth válida en apex (no `prompt=none`, que es OIDC-specific).
- **Cookie host-only del custom domain con JWT propio**: Feature C. V1 no toca cookies del custom domain.
- **Provisioning de `oauth_client_id`**: **NO aplica**. ADR-0032 deprecó la ruta OIDC canónica. La columna queda NULL indefinidamente (forward-compat: si V2 vuelve a OIDC canonical se reutiliza). ADR-0027 nunca se escribirá.
...
**Por qué no botón "Iniciar sesión local"**: no existe path local V1. El gate V1 es honestidad UX: "para administrar, andá a la URL canónica". Con Feature C V1 deployed (ADR-0032), el gate page evolucionó: **NO se muestra como branch primario**. Cuando el visitor llega sin sesión local, el layout server-side redirige a `/api/auth/sso-init` (silent SSO via redirect chain). Sólo cuando ese flow falla (con `?sso_error=<code>`) se renderiza `<SsoFallbackPanel>`, que reusa `<AuthGateForCustomDomain>` como CTA fallback ("andá al subdomain canon"). Sin `prompt=none` (OIDC-specific) — el flow es server-side redirects.
...
5. **Auth gap UX para owners**. Escenario: owner típico V1. Logea en `place.community/login`, navega a `nocodecompany.co/settings` esperando administrar. **Comportamiento V1**: gate page educativo + link al subdomain canónico. **Limitación conocida**: el owner debe hacer 1 click extra para llegar al settings. **Mitigation V1**: copy honesto + UX explícita (ADR-0031 §4). **Cierre estructural**: Feature C V1 deployed (ADR-0032, **Signed Ticket pattern**). El owner típico autenticado en apex (`place.community`) que navega a `nocodecompany.co/settings` ya no ve el gate page — el layout server-side dispara silent SSO (init→issue→redeem) y aterriza directamente en settings con sesión local (cookie `__Host-place_sso_session`). El gate page V1 (`<AuthGateForCustomDomain>`) queda como CTA fallback dentro de `<SsoFallbackPanel>` para los casos donde el silent SSO falla con `?sso_error=<code>` (state mismatch, replay, expired, etc — ver tabla de error codes en [`docs/features/custom-domain-sso/spec.md`](../custom-domain-sso/spec.md)).
...
- **Auth + OIDC + custom domains (canónica macro)**: [`docs/decisions/0001-auth-oidc-custom-domains.md`](../../decisions/0001-auth-oidc-custom-domains.md) — **refinada por** [ADR-0032](../../decisions/0032-custom-domain-sso-signed-ticket.md) (Signed Ticket pattern) y [ADR-0033](../../decisions/0033-apex-login-honors-returnto.md) (apex login honra `returnTo` durante cold-start SSO M1).
