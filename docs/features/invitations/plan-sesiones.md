...
| S2 (Sesión B) — `inviteContext` branding + toggle hide | ✓ done (con corrección operacional ADR-0046) | `baseline/feature-e-invite-v1.2-s-b-done` | (este commit) | Helper TS nuevo `src/shared/lib/invitation-preview-lookup.ts` (wrapper memoizado React.cache sobre `app.invitation_preview` DEFINER) + `<AccessFlow>` extendido con prop `inviteContext` (header branding + subtitle + toggle hide + acceptHint + override `onSuccess` al `postCredentialUrl`) + `AccessLabels` extendido con 3 keys opcionales + `/login` page wire-up (parseo `?invite=` + lookup + `buildPlaceCanonicalUrl` postCredentialUrl + pass-through al `<AccessFlow>`) + invite page wire-up `&invite=<token>` en login/signup URLs + i18n × 6 locales (3 keys nuevas, parity verde). Tests: 14 helper unit + 7 AccessFlow RTL nuevos + suite full verde + typecheck verde. **Corrección operacional**: ADR-0046 §D2 + §"Plan de implementación" mencionaban `/crear` como page tocada — error documental (verified vs código: `/crear` renderiza `<PlaceWizard>` no `<AccessFlow>`; ADR-0045 §D5 explícitamente "PlaceWizard intacto"; invite page no linkea a `/crear`). Sesión B implementa SÓLO `/login`; `/crear` intacto. Addendum operacional + corrección inline (footnote + strikethrough) en ADR-0046. LOC reales: ~98 code + ~140 tests vs ~70+~50 estimado — overshoot documentado en addendum. |
| S3 (Sesión C) — Silent SSO post-credential | ✓ done | `baseline/feature-e-invite-v1.2-s-c-done` | (este commit) | Helper TS nuevo `buildSsoInitUrlForInvite` en `src/shared/lib/auth-redirect.ts` (espejo estructural de `buildPlaceCanonicalUrl` con emit de `sso-init` URL en branch custom domain) + wire-up del `/login` page (`postCredentialUrl` ahora dispara silent SSO via `sso-init` Feature C cuando el place tiene custom domain) + JSDoc del componente reactualizada (sin "Sesión C extenderá"). Para places sin custom domain bit-exact comportamiento Sesión B (subdomain canon directo). Para places con custom domain cierra el gap UX: pre-Sesión C aterrizaba SIN sesión en custom domain, post-Sesión C la cadena init→issue→redeem mintea cookie local + redirige a `/invite/{token}` con sesión. Tests: 10 unit nuevos sobre el helper (custom + canon + normalizaciones + dev local + encoding + fail-safe). Suite full verde + typecheck + lint. LOC reales: ~57 helper + ~12 page + ~150 tests **bajo** estimado plan (150-200 + 100). Sub-caps SSO + custom-domain + invitations **sin cambio** ✓. Cero migration, cero cambio al contrato `sso-issue`. Addendum operacional Sesión C en ADR-0046 (sin corrección — D4 se materializa exacto como descrito). |
