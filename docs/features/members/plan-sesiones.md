...
**LOC budget estimado**: ≤920 LOC. **CHECK feature slice cap** al cierre S10 — si excede 1500, ejecutar Plan B split (`place-ownership-actions/` slice separado — nombre post-S10.6 ADR-0040; originalmente `members-ownership/` durante S10.5). **Actualización post S10.7 (ADR-0041)**: 2ª extracción ejecutada — `invitations/` (1497 LOC) escindido del slice padre. `members/` bajó de 3765 → 2237 LOC. **Actualización post S10.8 (ADR-0042)**: 3ª extracción ejecutada — `member-profile/` (589 LOC) escindido del slice padre. `members/` bajó de 2237 → 1678 LOC (-559 / -25%). Aún sobre cap por 178 LOC; S10.9 (refactor `member-row-actions-menu` a page-level con render-prop — no es extracción) cierra el gap restante. **Actualización post S10.9 (ADR-0043)**: cierre del slice diet ejecutado — `<MemberRowActionsMenu />` movido a page-level co-located (`src/app/(app)/place/[placeSlug]/settings/members/_components/`, 2 files `git mv` history 100% preservada) + `<MembersList />` refactor a render-prop puro (`renderRowActions?: (member) => ReactNode`). `members/` bajó de 1678 → **1074 LOC** (-604 / -36%, **426 LOC bajo cap**). **Total slice diet S10.5-S10.9**: 3765 → 1074 LOC (-2691 / **-71%**); 4 slices hermanos resultantes (members/ 1074 + place-ownership-actions/ 717 + invitations/ 1497 + member-profile/ 589) + 1 componente page-level (`_components/member-row-actions-menu.tsx` 309 + test 258 = 567 LOC). Suite verde 103/103 archivos · 932/932 tests (+2 por nuevos render-prop tests).
...
- **Perfil contextual del miembro vive en `src/features/member-profile/`** (slice hermano, S10.8 ADR-0042 extracción). Cohesión: capability autónoma del perfil contextual self-only (ADR-0036) — asimetría caller==target (vs owner→target de `removeMemberAction`) hace explícita la separación capability vs `members/` core. Contenido V1: `updateMyHeadlineAction` + `<HeadlineEditor />` + tipos (`HeadlineError`) + zod schema (`updateMyHeadlineSchema`). Reserva V1.1+: avatar contextual + otros campos perfil-en-place — el nombre cubre la capability completa, no requiere otra extracción. El page S11 será el 1er cross-slice consumer (importa desde `@/features/member-profile/public`). Members 2237 → 1678 LOC con esta extracción.
- **`<MemberRowActionsMenu />` vive a page-level co-located en `src/app/(app)/place/[placeSlug]/settings/members/_components/`** (S10.9 ADR-0043 — NO es slice ni vive en `features/`). Razón: ensambla 4 actions cruzando 2 slices (`members/` 1 + `place-ownership-actions/` 3) con un único page-consumer y sin DEFINER/spec propia ⇒ no califica como slice (ADR-0028 §"Política a futuro"). Inaugura convención `app/.../_components/` (Next.js `_*` privado / no-route) para page-level glue code que ensambla ≥2 slices. El page S11 lo inyecta a `<MembersList renderRowActions={(m) => <MemberRowActionsMenu member={m} ... />} />` (render-prop puro post-S10.9). Members 1678 → 1074 LOC con este movimiento (cierre del slice diet S10.5-S10.9: **-2691 LOC / -71% vs estado pre-diet**).
