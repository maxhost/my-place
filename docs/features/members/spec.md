...
3. **Aceptar invite**: en incognito, abrir el link copiado → page consumer del invite (Feature E V1.1, ver [`../invitations/spec.md`](../invitations/spec.md) §"Flow accept" + ADR-0044) → bob completa login/signup vía CTAs del page + click "Aceptar invitación" → bob aparece como miembro activo en la lista de alice (`/settings/members` reload). **Nota V1.1 (2026-05-26)**: V1 original asumía erróneamente "flow de Feature C" — Feature C es custom-domain-sso, NO signup/login. El consumer del token fue implementado en V1.1 como sesión propia post-V1 (gap detectado en smoke E2E `dpl_J5QcmBYmLQENi9yz8owK5j8eCt4N`).
