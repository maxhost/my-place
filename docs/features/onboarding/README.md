...
- Token: `await auth.token()` (endpoint `/token` del plugin JWT de Neon Auth) → **JWT**. **NO** `getSession().session.access_token` ni el token de `signUp` (sesión OPACA, no JWT) ni `auth.getAccessToken()` (OAuth de proveedor) — corregido por **ADR-0018** (verificado en prod 2026-05-19; el wording previo era incorrecto). Verificación con **`jose`** (`createRemoteJWKSet(new URL(NEON_AUTH_JWKS_URL))` + `jwtVerify`); el claim **`sub`** = `neon_auth.user.id`.
