...
baseline/feature-c-s11.3-done           = 1fd9160   (S11.3.D close T1.3: smoke M1 retry VERDE + docs close + push bundle A+B+C autorizado 2026-05-23 deploy dpl_FwjvKLuj9v9AmPM48ngrUwXU4Dpu READY en 43s — Feature C V1 cerrada end-to-end)
baseline/pre-docs-sweep                 = 1fd9160   (= feature-c-s11.3-done, save point pre-DS series 2026-05-23 — auditoría docs↔plan↔código post-V1)
```
...
- **Spec del feature**: [`./spec.md`](./spec.md) — §"Smoke ejecutado 2026-05-23" (T1.1 + T1.2 VERDE) + §"T1.3 inicial ROJO + S11.3 fix" + §"T1.3 retry post-fix" (VERDE, deploy `dpl_FwjvKLuj9v9AmPM48ngrUwXU4Dpu` commit `48b204b` + commit close `1fd9160`) + §"Conclusión final Feature C V1" (M1+M2+M3 cubiertos).
