> **Cierre operativo S11.3, 2026-05-23**: bundle A+B+C (commits `7d872ad` S11.3.A docs + `d03b30d` S11.3.B helper + `48b204b` S11.3.C wire) pusheado a `maxhost/main` con autorización explícita del user · deploy production `dpl_FwjvKLuj9v9AmPM48ngrUwXU4Dpu` (commit `48b204b`) READY en 43s con alias `nocodecompany.co` + `place.community` + `www.place.community` + `app.place.community` mapeados · **smoke server-side sanity 2/2 VERDE** (sso-issue sin cookie → 302 a apex login con `?returnTo=` preservado URL-encoded apuntando al sso-issue completo; login con returnTo válido → 200 + form HTML 14460 bytes) · **smoke M1 owner-driven VERDE** (incógnito limpio sin cookies Place previas → entry `nocodecompany.co/settings` → redirect a apex login → identificación con credenciales owner → vuelve transparente a `nocodecompany.co/settings` con sesión SSO local) · cita literal del user: _"paso perfectamente. entre a nocodecompany.co/settings me dirigio al login de place.community despues de identificarme me redirigio a nocodecompany.co/settings"_. Cold-start SSO M1 cerrado end-to-end · Feature C V1 cubre los 3 escenarios canónicos (M1 anónimo + M2 sesión activa + M3 expirados subsumido por M1). Bug T1.3 (apex login descarta `?returnTo`) cerrado. Tag final `baseline/feature-c-s11.3-done` = commit close S11.3.D (este write-back). Detalle completo: `docs/features/custom-domain-sso/spec.md` §"T1.3 retry post-fix" + §"Conclusión final Feature C V1".
...
- **Spec del feature**: `docs/features/custom-domain-sso/spec.md` §"T1.3 inicial ROJO" + §"S11.3 fix" (write-back en S11.3.A) + §"T1.3 retry post-fix VERDE" (write-back en S11.3.D, evidencia 2026-05-23 deploy `dpl_FwjvKLuj9v9AmPM48ngrUwXU4Dpu` commit `48b204b`) + §"Conclusión final Feature C V1".
