...
> **Cierre operativo Feature C V1 — 2026-05-23 (end-to-end T1.1 + T1.2 + T1.3)**: deployed verde en producción cubriendo los 3 escenarios canónicos del cold-start SSO desde custom domain (M1 anónimo + M2 sesión apex activa + M3 expirados subsumido por M1). Tag baseline final: **`baseline/feature-c-s11.3-done`** = commit **`1fd9160`** (S11.3.D close).
>
> **Sub-bugs cerrados** (cada uno con su fix + write-back):
> 1. **T1.1** silent SSO `signature_invalid` (M2) → cerrado por **S11.1** (deploy `dpl_5fmp8Lfc7sagPiB8bPaGmJZ2dXM4` READY, commit `473c3e8`). Fix Opción D (customFetch + same-registrable-domain allowlist) documentado en §12 + gotcha `docs/gotchas/jose-jwks-redirect-manual.md`.
> 2. **T1.2** 4 Server Actions broken-on-custom-domain por RFC 6265 cookie scope (M2) → cerrado por **S11.2** (deploy `dpl_2vhnAC2REbcjGgureWp85VRqpzj6` READY, commit `5e62f0d`/`17b5df5`). Fix Opción B (helper coordinador zone-aware `getAuthenticatedDbForRequest` + 4 Server Actions migradas) documentado en `docs/features/custom-domain-sso/spec.md` §"S11.2 fix Opción B".
> 3. **T1.3** apex login descarta `?returnTo` en cold-start M1 → cerrado por **S11.3** (deploy `dpl_FwjvKLuj9v9AmPM48ngrUwXU4Dpu` READY en 43s, commit `48b204b` wire + `1fd9160` close). Fix Opción única (helper PURE `validateLoginReturnTo` + wire-up minimal en page apex + `AccessFlow`) documentado en **ADR-0033** + gotcha `docs/gotchas/apex-login-returnto-honored.md`.
>
> **Sub-cap LOC `src/shared/lib/sso/`** post-Feature-C V1: **1400 LOC** (bumps elásticos documentados en §5 addenda: 800 [original] → 1000 [S3.5] → 1100 [S11.1] → 1400 [S11.3.B]).
>
> Continuidad RLS verificada empíricamente: el `sub` del local session JWT === `neon_auth.user.id` → `app.current_user_id()` retorna el mismo valor en custom domain que en apex (ADR-0032 §6 cumplido end-to-end). Write-back completo en [`../features/custom-domain-sso/plan-sesiones.md`](../features/custom-domain-sso/plan-sesiones.md) (con SHAs reales) + smoke evidence en [`../features/custom-domain-sso/spec.md`](../features/custom-domain-sso/spec.md) §"Smoke ejecutado 2026-05-23" + §"T1.2 retry post-fix" + §"T1.3 retry post-fix" + §"Conclusión final Feature C V1".

- **Fecha:** 2026-05-22 · **Cierre operativo:** 2026-05-23
- **Estado:** Aceptada · **V1 deployed**
- **Alcance:** auth cross-domain (custom domains que no comparten cookie con el apex `place.community`) · módulo nuevo `src/shared/lib/sso/` (sub-cap LOC 800 propio en V1; bumps elásticos post-deploy → 1400 LOC, ver §5 addenda) · 4 endpoints API nuevos (`/api/auth/sso-init`, `/api/auth/sso-issue`, `/api/auth/sso-redeem`, `/api/auth/sso-jwks`) · migration 0011 (función `app.consume_sso_jti` + tabla `app.sso_jti_used`) · 2 cookies nuevas (`__Host-place_sso_state` + `__Host-place_sso_session`) · componente nuevo `<SsoFallbackPanel>` del slice existente `custom-domain-routing` · adaptación de `getSessionTokenForZone` + `getPlaceForZone` (single-callers locales) · **2 env vars nuevas** (`PLACE_SSO_SIGNING_KEY` PEM PKCS8 ES256, `PLACE_SSO_SIGNING_KEY_KID` string corto)
...
- **Refinada por:** ADR-0033 §"Wire-up apex login `searchParams.returnTo`" (2026-05-23, S11.3) — cierra el contrato del lado consumidor del `?returnTo` que §2 step 2 (apex `sso-issue` redirige a `/login?returnTo=<sso-issue URL>` cuando no hay sesión Neon Auth) asumía implícito: el login apex `/[locale]/login` ahora honra ese query param sanitizado server-side via helper PURE `validateLoginReturnTo` (allowlist `/api/auth/sso-{issue,init}` + same-registrable-domain HTTPS) + propaga al `AccessFlow` que lo navega tras submit exitoso. Cierra cold-start SSO M1 (visitor anónimo en `nocodecompany.co/settings`); backwards-compat preservada (flows sin returnTo siguen al Hub canónico hardcoded).
...
- **`GET /api/auth/sso-redeem`** (montado en **custom domain**): redime el ticket. Query: `?ticket=<jwt>&state=<>&returnTo=<>`.
  ...
  4. `verifySsoTicket(ticket, host, jwks)` → throws con `SsoTicketError` mapeado a `?sso_error=<code>`. Códigos canónicos del `SsoTicketError.code` discriminado: `expired` | `aud_mismatch` | `iss_mismatch` | `signature_invalid` | `missing_claim` | `jwt_malformed` (definidos en `src/shared/lib/sso/sso-ticket.ts:79-92`).
...
**Consumer canónico**: `getPlaceForZone` (en `src/app/(app)/place/[placeSlug]/_lib/get-place-for-zone.ts`, S9 wire-up) llama `getAuthenticatedDbWithVerifier(sessionToken, verifyLocalSession, fn)` directamente cuando `hostZone.zone === 'custom-domain'`. **V1 no necesita wrapper conveniente adicional** — el único caller del path SSO local es `getPlaceForZone`, llamada directa = simplest fit (no se reproyectó `getCustomDomainDb(sessionToken, fn)` propuesto en plan original porque no agregaba valor sobre la llamada directa de 2 args). Post-S11.2 también lo usa indirectamente vía `getAuthenticatedDbForRequest` (helper zone-aware, ver `docs/features/custom-domain-sso/spec.md` §"S11.2 fix Opción B" — dispatcha al `getAuthenticatedDbWithVerifier` cuando detecta zone custom-domain).
