...
> **Cierre operativo 2026-05-23**: Feature C V1 deployed end-to-end verde. Commit final `473c3e8` (S11.1 docs close, post-fix Opción D). Deploy production: `dpl_5fmp8Lfc7sagPiB8bPaGmJZ2dXM4` READY. Smoke T1.1 retry verde con evidencia cookie `__Host-place_sso_session` correctamente seteada (claims `iss/sub/host/iat/exp` válidos, continuidad RLS empíricamente verificada). Tag baseline final: `baseline/feature-c-done`. Postmortem T1.1 bug + fix Opción D documentados en §12 + addendum sub-cap 1000→1100 §5 + gotcha `docs/gotchas/jose-jwks-redirect-manual.md`. Write-back completo en [`../features/custom-domain-sso/plan-sesiones.md`](../features/custom-domain-sso/plan-sesiones.md) (con SHAs reales) + smoke evidence en [`../features/custom-domain-sso/spec.md`](../features/custom-domain-sso/spec.md) §"Smoke ejecutado 2026-05-23".
