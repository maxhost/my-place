...
- **Customización activa, no algorítmica.** El owner del place configura colores. El orden y la personalización son decisión humana, no del algoritmo. _Aclaración (ADR-0005, ajustada por ADR-0007):_ el LLM del onboarding puede **proponer** un punto de partida (paleta + borrador de descripción; **no** horario), pero la decisión es humana — el owner edita y confirma; nada se auto-aplica. Propose-only + confirm obligatorio mantiene el principio; auto-aplicar salida de LLM sin confirmación lo violaría y está prohibido.
