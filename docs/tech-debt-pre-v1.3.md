...
- [x] Configurar `pnpm test --coverage` (vitest v8) con thresholds → **HECHO 2.C.3, 2026-06-02**: `@vitest/coverage-v8@4.1.6` (pin = vitest), `coverage` a nivel raíz en `vitest.config.ts` (merge nativo de ambos projects), script `test:coverage` = `vitest run --coverage`. Reporters `text`+`json`+`json-summary`+`html`. **Thresholds calibrados contra medición real** (piso de no-regresión, nunca por encima de lo medido):
  - **Global**: statements/branches/lines **70** (medido 70.78 / 72.64 / 71.59 ✓), functions **68** (medido **69.46**, sub-70 por un pelo → pinearlo en 70 rompería ante fluctuación mínima; subir cuando lleguen funcs nuevas con tests).
  - **`src/features/access/**`**: **85** las 4 métricas → target del tracker **CUMPLIDO con holgura** (medido 94.44 / 90.00 / 95.45 / 95.92).
  - **`src/features/invitations/**`**: **85** las 4 métricas → target del tracker **CUMPLIDO** (medido 92.20 / 87.61 / 90.32 / 95.68 tras escribir los unit mockeados de los 3 wrappers, ver bullet abajo). Pre-tests estaba en 62-68% (gap 100% en los wrappers: 0 funciones cubiertas). `branches` 87.61 es la métrica más ajustada.
- [x] **Unit tests de los 3 wrappers de action de `invitations`** (cerró el gap → 85, decisión del user "escribir ahora" en vez de diferir) → **HECHO 2.C.3, 2026-06-02** (27 tests, mismo patrón mockeado que `auth-actions.test.ts`):
  - [x] `actions/__tests__/accept-invitation.test.ts` (12 tests): happy (TX1 ensureAppUser + TX2 DEFINER → slug), Zod fail, rate_limited + derivación retryAfterSeconds (cap 3600 / piso 1), identity null → unauthenticated, rows vacías → unknown, ensureAppUser P0002 → app_user_missing, DEFINER P0005/P0007/desconocido → map por SQLSTATE. Mock de `getCurrentUserIdentityForRequest`/`getAuthenticatedDbForRequest`/`ensureAppUser`/`enforceRateLimit`/`getRequestIp`.
  - [x] `actions/__tests__/create-invitation.test.ts` (9 tests): happy + revalidatePath del place + identifier compuesto IP:place, Zod triage (invalid_email / invalid_expires / generic), rate_limited, sin payload → generic, DEFINER not_owner / 28000→unauthorized / desconocido→generic. Mock de `getAuthenticatedDbForRequest`/`enforceRateLimit`/`getRequestIp`/`next/cache`.
  - [x] `actions/__tests__/revoke-invitation.test.ts` (6 tests): happy + revalidatePath, Zod fail → generic, DEFINER not_found / already_accepted / 28000→unauthorized / not_owner / desconocido→generic. Mock de `getAuthenticatedDbForRequest`/`next/cache`. (Sin rate limit: la action no lo aplica.)
  - **Patrón clave**: el mock de `getAuthenticatedDbForRequest` INVOCA el callback (`fn(sqlExecutor, {sub})`) para ejecutar la línea `sql(…)` real → cubre el wrapper, no sólo lo stubea. Happy vía `sqlExecutor.mockResolvedValue`, errores vía `mockRejectedValueOnce({code})` (espeja el SQLSTATE de la DEFINER).
- [x] Comment coverage en PRs → **HECHO 2.C.3**: job `coverage` en `.github/workflows/tests.yml` con `needs: vitest` (corre tras el gate bloqueante → sin carga concurrente sobre el branch `test` de Neon, evita la presión de conexiones que la gotcha 2.C.1 deja como causa probable del flake) + `permissions: pull-requests: write` + `davelosert/vitest-coverage-report-action@v2` (lee `coverage-summary.json` + `coverage-final.json`, `if: always()` para postear el reporte aun si un threshold rompe). Verificado local: `pnpm test:coverage` **1226/1226 exit 0**, thresholds verdes end-to-end (global 72.79/74.08/70.59/73.71 · access 90-96 · invitations 87-96).
- [x] **Fix timeout bajo coverage** (hallazgo de 2.C.3, reproduce el flake de 2.C.1): el full run con instrumentación v8 timeouteó 71 tests de `src/db/__tests__/*` (`Hook timed out in 30000ms`, intermitente) — la instrumentación ensancha el cold-connect de Neon por encima del sobre de 30s. Fix `vitest.config.ts`: `nodeDbTimeout = underCoverage ? 60_000 : 30_000` (detecta `COVERAGE=1` del script o `--coverage`); runs normales `test:node`/`test:ui` quedan en 30s. Documentado en `docs/gotchas/vitest-parallel-projects-flake.md` §"Reaparición bajo coverage". Re-verificado: 1226/1226 verde a 60s. **Acota la causa del flake original**: contención del cold-connect bajo carga (no CPU-starvation, ya refutada), confirmada al reproducirla determinísticamente bajo instrumentación.
- [x] Unit tests faltantes (cubrir branches no testeadas) → **HECHO 2.C.2, 2026-06-02**:
  - [x] `src/features/nav-hub/actions/logout-action.ts` — `logout-action.test.ts` (7 tests): locale válido (signOut + URL apex localizada), todos los locales soportados, best-effort si signOut lanza (no propaga), open-redirect guard (locale inválido → fallback `routing.locales[0]`, sin doxxear el flag), variantes de inyección. Mock de `getAuth`/`rootDomain` → sin DB ni SDK vivo.
  - [x] `src/features/access/auth-actions.ts` — `auth-actions.test.ts` (15 tests): `loginAction` (happy + propaga parseados, Zod fail email/password, rate_limited + derivación retryAfterSeconds [cap 3600 / piso 1], SDK `{error}`, SDK throw) + `signUpAccountAction` (happy + propaga name, Zod fail email, displayName whitespace-only post-trim, trim del name, rate_limited, SDK `{error}`, `data.token` falsy, `data` null [`data?.token`], SDK throw). Mock de `getAuth`/`enforceRateLimit`/`getRequestIp` → branches TS aisladas (wiring vivo se verifica en preview, comment `auth-actions.ts:50-59`).
  - [x] Integration `app.invitation_preview` directo (no solo via wrapper) → **YA CUBIERTO** en `src/db/__tests__/accept-invitation.test.ts` §"S6 app.invitation_preview" (token válido sin-claim + anti-mutación, inexistente, vencido, ya usado, EXECUTE app_system / PUBLIC denegado). Diagnose-before-implement: no se duplica.
  - [x] ~~Integration `app.lookup_user_email_by_id`~~ → **SKIP**: función dropeada (migrations 0023→0026, confirmado en 2.D), decisión 1.A.iii = drop.
