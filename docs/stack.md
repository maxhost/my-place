...
| Pagos          | **Stripe Connect** (dirección, ADR-0053)        | Dos ejes ortogonales: (a) suscripción de oyentes → cuenta Stripe **del creador** vía Connect, **0% para Place** (desbloquea threads privados, `docs/ontologia/monetizacion.md`); (b) suscripción SaaS creador→Place (revenue de Place; pricing TBD). ADR-0005/0003 siguen fijando el arranque del eje (b): trial 30d (`place.trial_ends_at`) → paywall `PAYMENT_PENDING`. El enum `billing_mode` quedó superseded (columna dormida). Integración concreta (Connect onboarding, webhooks, schema de suscripción): al spec del feature. |
...
- **Custom domains (Vercel API)**: `VERCEL_API_TOKEN` (scope `domains:write`) + `VERCEL_PROJECT_ID`. Sin estas el slice `/settings/domain` degrada (defensivo, no crash). Consumidas por `src/shared/lib/vercel/domains.ts` (ADR-0026). `VERCEL_API_BASE_URL` (opcional, default `https://api.vercel.com`): override del host destino — seam DI usado SÓLO por los E2E (Phase 2.B.1) para apuntar el wrapper a un stub HTTP local (`scripts/e2e-vercel-stub.mjs`); producción nunca la setea. Ver `docs/testing.md` §"Mock de Vercel en E2E".
...
**Realtime** sigue TBD — se agrega cuando se decida. **Pagos** con dirección resuelta (Stripe Connect, ADR-0053): las env vars de Stripe se agregan al construir el feature monetización. **Storage** RESUELTO en Phase 1.G (ADR-0048, Cloudflare R2). **Todo lo que sea secret** (`*_SECRET`, `*_API_KEY`, `*_TOKEN`, `DATABASE_URL*`, `R2_SECRET_ACCESS_KEY`) vive solo en `.env.local` / Vercel env, **nunca en git**.
