...
- **Sesión:** un custom domain no comparte la cookie del apex Neon Auth (`Domain=.place.community`, host-only por dominio). **V1 (Feature B, ADR-0031 §4): gate page educativa server-side** (`<AuthGateForCustomDomain>`, slice `custom-domain-routing`) — owners en custom domain sin sesión local ven copy localizado en `place.default_locale` con link explícito al subdomain canónico. Sin auto-redirect (UX agency, no loop perceptual). **V2 / Feature C** cierra el gap con OIDC SSO + cookie host-only del custom domain con JWT propio + silent SSO via `prompt=none` para owners ya autenticados en apex. Canónico macro en ADR-0001; cierre operativo cuando Feature C entre.
